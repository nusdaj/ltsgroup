...
class ControllerQuickCheckoutShippingMethod extends Controller {
  	public function index() {
		...
		if ($this->customer->isLogged() && isset($this->request->get['address_id'])) {
			...
			if($this->request->post['address_1'] == ""){
			    if (isset($this->session->data['fds']['address_1'])) {
        			$shipping_address['address_1'] = $this->session->data['fds']['address_1'];
			    }else{
			...
			if($this->request->post['address_2'] == ""){
			    if (isset($this->session->data['fds']['address_2'])) {
        			$shipping_address['address_2'] = $this->session->data['fds']['address_2'];
			    }else{
			...
			if($this->request->post['postcode'] == ""){
			    if (isset($this->session->data['fds']['postal_code'])) {
        			$shipping_address['postcode'] = $this->session->data['fds']['postal_code'];
			    }else{
		...
		$data['delivery_times_lalamove'] = $this->config->get('quickcheckout_delivery_times_lalamove');
        ...
        $cutoff =  $this->config->get('quickcheckout_delivery_cut_off');
		if(strtotime(date("Y-m-d H:i:s")) > strtotime($cutoff)){
			$data['delivery_min'] = date('Y-m-d', strtotime('+' . $this->config->get('quickcheckout_delivery_min') + 1 . ' days'));
		}
  	...
  	public function setTiming(){
  	    $this->session->data['lalamove_time'] = $this->request->post['selected_time'];
  	}
