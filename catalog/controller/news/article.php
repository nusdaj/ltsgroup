...
class ControllerNewsArticle extends Controller {
	...
	protected function getPageContent($news_info) {
		...
		if (isset($this->request->get['ncat'])) {
			$data['artlink'] = $this->url->link('news/article', 'ncat=' . urlencode(html_entity_decode($this->request->get['ncat'], ENT_QUOTES, 'UTF-8')) . '&news_id=' . $news_id);
		} else {
			$data['artlink'] = $this->url->link('news/article', 'news_id=' . $news_id);
		}
