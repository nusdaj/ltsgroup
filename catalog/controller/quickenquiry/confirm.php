<?php 
class ControllerQuickenquiryConfirm extends Controller { 
	public function index() {
		...
		if ($this->enquiry->hasShipping()) {
			...
			if (!isset($this->session->data['shipping_address'])) {
				$redirect = $this->url->link('enquiry/checkout', '', true);
			}
		...
		if ( !$this->enquiry->hasProducts() || (!$this->enquiry->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$redirect = $this->url->link('enquiry/cart');
		}
...
*/
			...
			(!isset($this->session->data['reward']) || (int)$this->session->data['reward'] < 1) 
				...
				$this->load->model('extension/enquiry_total/reward');

				$reward_info = $this->model_extension_enquiry_total_reward->getRewardInfoByCustomerGroup($customer_group_id);
			...
			*/
			...
			$this->session->data['enquiry_order_id'] = $this->model_checkout_enquiry->addOrder($order_data);
			...
			foreach ($this->enquiry->getProducts() as $product) {
				...
				$data['products'][] = array(
					...
					'enquiry_id' => isset($product['enquiry_id']) ? $product['enquiry_id'] : $product['key'],
					'product_id' => $product['product_id'],
					'name'       => $product['name'],
					'model'      => $product['model'],
					'option'     => $option_data,
		...
		$data['back'] = $this->url->link('quickenquiry/checkout', '', true);
		
		$this->response->setOutput($this->load->view('quickenquiry/confirm', $data));
  	}
}