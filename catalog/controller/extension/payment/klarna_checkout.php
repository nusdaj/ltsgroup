...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	private function klarnaOrderData($klarna_account) {
		...
		$merchant_urls = array(
			'checkout'	     => html($this->url->link('extension/payment/klarna_checkout', 'klarna_order_id={checkout.order.id}', true)),
			'confirmation'   => html($this->url->link('extension/payment/klarna_checkout/confirmation', 'klarna_order_id={checkout.order.id}', true)),
			'push'			 => html($this->url->link('extension/payment/klarna_checkout/push', 'klarna_order_id={checkout.order.id}', true)),
			'validation'	 => html($this->url->link('extension/payment/klarna_checkout/validation', 'klarna_order_id={checkout.order.id}', true)),
			'address_update' => html($this->url->link('extension/payment/klarna_checkout/addressUpdate', 'klarna_order_id={checkout.order.id}', true)),
		);

		if ($this->config->get('klarna_checkout_terms')) {
			$merchant_urls['terms'] = html($this->url->link('information/information', 'information_id=' . $this->config->get('klarna_checkout_terms'), true));
		}
