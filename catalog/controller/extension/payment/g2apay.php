...
class ControllerExtensionPaymentG2APay extends Controller {
	...
	public function checkout() {
		...
		$string = $this->session->data['order_id'] . $order_total . $order_info['currency_code'] . html($this->config->get('g2apay_secret'));
	...
	public function ipn() {
		...
		if (isset($this->request->get['token']) && hash_equals($this->config->get('g2apay_secret_token'), $this->request->get['token'])) {
			...
			if (isset($this->request->post['userOrderId'])) {
				...
				$string = $g2apay_order['g2apay_transaction_id'] . $g2apay_order['order_id'] . round($g2apay_order['total'], 2) . html($this->config->get('g2apay_secret'));
				$hash = hash('sha256', $string);
