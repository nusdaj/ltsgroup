...
class ModelCatalogCategory extends Model {
	...
	// AJ added Apr 8: begin function, Get the number of products under one category
	public function getCategoryNoProducts($category_id) {
		$query = $this->db->query("SELECT COUNT(product_id) AS total FROM " . DB_PREFIX . "product_to_category AS pc CROSS JOIN " . DB_PREFIX . "category_path AS cp ON pc.category_id = cp.category_id WHERE path_id = '" . (int)$category_id . "' GROUP BY path_id");
