...
<div<?php echo $delivery_required ? ' class="required"' : ''; ?>>
  ...
  <?php if ($delivery_delivery_time == '3') { ?><br />
    <select name="delivery_time" class="form-control selected_delivery_time">
        <?php foreach ($delivery_times as $key => $quickcheckout_delivery_time) { ?>
            <?php if (!empty($quickcheckout_delivery_time[$language_id])) { ?>
              <?php if ($delivery_time == $quickcheckout_delivery_time[$language_id]) { ?>
        	    <option value="<?php echo $quickcheckout_delivery_time[$language_id]; ?>" selected="selected" data-time="<?= $delivery_times_lalamove[$key][1]; ?>"><?php echo $quickcheckout_delivery_time[$language_id]; ?></option>
        	  <?php } else { ?>
        	    <option value="<?php echo $quickcheckout_delivery_time[$language_id]; ?>" data-time="<?= $delivery_times_lalamove[$key][1]; ?>"><?php echo $quickcheckout_delivery_time[$language_id]; ?></option>
              <?php } ?>
        	<?php } ?>
        <?php } ?>
    </select>
...
<?php } ?>
$('.selected_delivery_time').on('change',function(){
    var time = $(this).find(':selected').attr('data-time');
    $.ajax({
		url: 'index.php?route=quickcheckout/shipping_method/setTiming',
		type: 'post',
