...
class ModelSaleVoucherTheme extends Model {
	public function addVoucherTheme($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "voucher_theme SET image = '" . $this->db->escape($data['image']) . "', type = '" . $this->db->escape($data['type']) . "', amount = '" . $this->db->escape($data['amount']) . "', sort_order = '" . $this->db->escape((int)$data['sort_order']) . "', seo_keyword = '" . $this->db->escape($data['seo_keyword']) . "'");
		...
		if ($data['seo_keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'voucher_theme_id=" . (int)$voucher_theme_id . "', keyword = '" . $this->db->escape($data['seo_keyword']) . "'");
		}
	...
	public function editVoucherTheme($voucher_theme_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "voucher_theme SET image = '" . $this->db->escape($data['image']) . "', type = '" . $this->db->escape($data['type']) . "', amount = '" . $this->db->escape($data['amount']) . "', sort_order = '" . $this->db->escape((int)$data['sort_order']) . "', seo_keyword = '" . $this->db->escape($data['seo_keyword']) . "' WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'voucher_theme_id=" . (int)$voucher_theme_id . "'");
