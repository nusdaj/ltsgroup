...
class ModelExtensionPaymentG2aPay extends Model {
	...
	public function refund($g2apay_order, $amount) {
		if (!empty($g2apay_order) && $g2apay_order['refund_status'] != 1) {
			...
			$string = $g2apay_order['g2apay_transaction_id'] . $g2apay_order['order_id'] . round($g2apay_order['total'], 2) . $refunded_amount . html($this->config->get('g2apay_secret'));
	...
	public function sendCurl($url, $fields) {
		...
		$auth_hash = hash('sha256', $this->config->get('g2apay_api_hash') . $this->config->get('g2apay_username') . html($this->config->get('g2apay_secret')));
