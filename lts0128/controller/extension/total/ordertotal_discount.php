...
class ControllerExtensionTotalOrdertotalDiscount extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('ordertotal_discount', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/ordertotal_discount', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/total/ordertotal_discount', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/total/ordertotal_discount.tpl', $data));
	}
	...
	public function install() {
		...
		$this->db->query("INSERT INTO `". DB_PREFIX ."setting` (`setting_id`, `store_id`, `code`, `key`, `value`, `serialized`) VALUES (NULL, '0', 'ordertotal_discount', 'ordertotal_discount_sort_order', '2', '0'); ");
	...
	public function uninstall() {
		...
		$this->db->query("DELETE FROM `". DB_PREFIX ."setting` WHERE `code` = 'ordertotal_discount';");
