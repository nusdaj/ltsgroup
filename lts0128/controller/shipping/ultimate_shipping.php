...
class ControllerShippingUltimateShipping extends Controller {
	...
	public function index() {
		...
		$data['rule_options'] = array(
			...
			'order_criteria'			=> array('currency', 'custom_field', 'customer', 'customer_group', 'language', 'past_orders', 'store'),
		...
		// Extension-specific
		$setting_override_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE (`" . (version_compare(VERSION, '2.0.1', '<') ? 'group' : 'code') . "` IN (SELECT `code` FROM `" . DB_PREFIX . "extension` WHERE `type` = 'shipping') AND `" . (version_compare(VERSION, '2.0.1', '<') ? 'group' : 'code') . "` != '" . $this->name . "') OR `key` = 'config_address' ORDER BY `" . (version_compare(VERSION, '2.0.1', '<') ? 'group' : 'code') . "`, `key` ASC");
		...
		foreach (array('cheapest', 'prediscounted', 'nondiscounted', 'taxed', 'total') as $total_value) {
		...
		foreach (array('shipping', 'payment') as $extension_type) {
			...
			foreach ($extension_query->rows as $extension) {
				...
				// extension-specific
				if ($extension['code'] == 'flat') continue;
				if ($extension['code'] == 'free') continue;
				if ($extension['code'] == 'item') continue;
				if ($extension['code'] == 'pickup') continue;
				if ($extension['code'] == 'weight') continue;
				// end
		...
		foreach ($this->getTableRowNumbers($data, $table, $sortby) as $num => $rules) {
			...
			// Extension-specific
			$charge_options['text_other_shipping_methods']	= '';
			foreach ($data['shipping_extension_array'] as $shipping_code => $shipping_name) {
				$charge_options['extension_' . $shipping_code] = $shipping_name;
			}
			// end
